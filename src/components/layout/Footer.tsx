...
export function Footer({ children }: FooterProps = {}) {
  ...
  useEffect(() => {
    ...
    const isMixed = document.documentElement.classList.contains("mixed");
    if (isDark) {
      setTheme("dark");
    } else if (isMixed) {
      setTheme("mixed");
              ...
              {!mounted ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className={`relative px-4 py-1.5 text-xs font-black rounded-full transition-colors duration-200 flex items-center gap-1.5 focus:outline-none z-10 cursor-pointer ${
                      theme === "mixed" ? "text-foreground" : "text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    {theme === "mixed" && (
                      <motion.div
                    ...
                    <span className="relative z-10">Mieszany</span>
                  </button>
                </>
              )}
            </div>
