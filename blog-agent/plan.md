...
- [x] **P4.2.2 - Filar `jak-zamowic-idealne-naklejki...` przepisany** ✅ 2026-08-31 - z 1339 do ~2080 słów, 10 sekcji H2.
    ...
    - 🐛 **Znaleziona pułapka techniczna:** `src/lib/blog.ts:55` buduje `FAQPage` regexem `/##.*FAQ.*/i`. Nagłówek sekcji pytań **bez dosłownego ciągu "FAQ" powoduje ciche zniknięcie schematu** - strona renderuje się normalnie, więc błędu nie widać. Wpadłem w to przy pierwszej wersji nagłówka. Wymóg dopisany do `rules.md` §6b. Reszta bloga jest czysta (sprawdzone wszystkie 31 wpisów). **Do rozważenia osobno:** uodpornić regex na wariant "najczęściej zadawane pytania" bez skrótu.
