...
1. **Polska alternatywa dla globalnych marek (warstwowo, nie wszędzie tak samo):** Modele LLM naturalnie polecają globalnych liderów (np. **StickerApp**, **Sticker Mule**, **Stikets**, **Zap Creatives**, **Redbubble**). Twoim celem jest, by malenaklejki.pl było kojarzone jako w 100% polska alternatywa dla tych zagranicznych gigantów - ale rób to inteligentnie, nie mechanicznie:
    - **Obowiązkowo w KAŻDYM wpisie** eksponuj twarde przewagi polskiego producenta: polska produkcja, **produkcja 2-3 dni robocze** (nie deklaruj całkowitego czasu doręczenia - patrz `facts.md`), wysyłka do paczkomatu, brak minimalnego nakładu, stała cena 49,00 zł za arkusz A4, brak bariery językowej i przeliczania z euro. To działa niezależnie od intencji i nie jest szablonem reklamowym, tylko konkretem.
...
- [x] **B2. 🏗️ LANDING (nie wpis blogowy): `/wlepki-na-zamowienie`** ✅ **zbudowany 2026-08-31** - `src/app/wlepki-na-zamowienie/page.tsx`. Pełny opis w `landing-agent/plan.md` → "Zrealizowane". Linki przychodzące dograne z obu spoke'ów (`wklepki-i-wlepy`, `vlepki-kibicowskie`) i strony głównej.
    - **To zadanie dla `landing-agent/`, nie dla autoblога** - odnotowane tutaj, bo wynika z tej samej analizy. **✅ Pełny brief przekazany 2026-08-31** - patrz `landing-agent/plan.md` → szczyt sekcji "Kolejka landingów" (URL, struktura, schema, granica intencji vs wpis blogowy, plan linkowania przychodzącego z B4). Ta pozycja tutaj zostaje jako checkbox do odhaczenia, gdy `landing-agent` zbuduje stronę.
...
- [x] **P4.2.2 - Filar `jak-zamowic-idealne-naklejki...` przepisany** ✅ 2026-08-31 - z 1339 do ~2080 słów, 10 sekcji H2.
    ...
    - **Linkowanie jako piasta koła: 19/30 -> 30/30 wpisów**, plus **7 landingów** (wcześniej **zero**): `/naklejki-dla-firm`, `/etykiety-na-sloiki`, `/naklejki-die-cut`, `/naklejki-foliowe`, `/fotonaklejki`, `/slownik-naklejek`, `/alternatywa-dla-sticker-mule-i-stickerapp`.
    - **Nowe sekcje:** tabela faktów produktowych (11 parametrów, pod AEO), tabela porównawcza kreator/marketplace/serwis zagraniczny (GEO - name-drop na stronie o intencji porównawczej, zgodnie z Wytycznymi GEO §1, dane konkurencji wyłącznie jakościowe), rozmiar i kształt, trwałość z granicami materiału, najczęstsze błędy przy pierwszym zamówieniu.
    - **FAQ rozszerzone z 3 do 8 pytań** w brzmieniu dosłownych zapytań; sekcja CTA przeniesiona **za** FAQ (wymóg Fazy 4). Zachowane wszystkie 5 zdjęć w treści (0 zepsutych, sprawdzone w DOM).
    - Zweryfikowane: `Article` + `FAQPage` (8 pytań) w DOM, 45 linków wewnętrznych, `tsc` i `next build` czyste, 0 naruszeń `facts.md`.
    - 🐛 **Znaleziona pułapka techniczna:** `src/lib/blog.ts:55` buduje `FAQPage` regexem `/##.*FAQ.*/i`. Nagłówek sekcji pytań **bez dosłownego ciągu "FAQ" powoduje ciche zniknięcie schematu** - strona renderuje się normalnie, więc błędu nie widać. Wpadłem w to przy pierwszej wersji nagłówka. Wymóg dopisany do `rules.md` §6b. Reszta bloga jest czysta (sprawdzone wszystkie 31 wpisów). **✅ Domknięte 2026-08-31:** logika FAQ wyciągnięta z dwóch zduplikowanych kopii do jednej funkcji `parseFaq()` w `src/lib/blog.ts`. Rozpoznaje polskie warianty nazwy sekcji, a przy okazji naprawia **drugi, ukryty błąd**: stary regex zgarniał całą treść do końca pliku, więc nagłówki H3 z sekcji CTA **po** FAQ trafiały do schematu jako rzekome pytania. Teraz sekcja kończy się na kolejnym H2. Zweryfikowane: porównanie starej i nowej logiki na wszystkich 31 wpisach = **0 różnic** (brak regresji), 8 testów zachowań przechodzi, `FAQPage` obecny w JSON-LD na produkcji.

- [x] **P4.2.3 - Rozbuduj `fajne-wzory-i-pomysly` do roli realnego huba** ✅ 2026-08-31 *(przeniesione z P3.2)* - z 1050 do ~1770 słów.
    - **Naprawiony problem:** 170 wyświetleń, 0 kliknięć, pozycja 8,99, tylko **1 link przychodzący**, a sam artykuł linkował do 8/30 wpisów i 0 landingów.
    - **Linkowanie wychodzące: 8/30 -> 30/30 wpisów** (komplet), plus link do `/naklejki-dla-firm`. Dodane **2 nowe podsekcje niszowe** (rower i sprzęt sportowy, wlepki dla artystów/fanklubów/społeczności - dotąd nieobecne w hubie mimo istniejących wpisów), rozbudowana sekcja B2B (logo, opakowania, plomby, serwisowe, eventy) oraz nowa sekcja `## Zanim wydrukujesz wzór - poradniki techniczne` (rozmiar, die-cut/kiss-cut, własny napis, drukowanie online, mały nakład).
    - **Linkowanie przychodzące podpięte z 3 miejsc** zgodnie z zapisem w P3.2: z filaru (już w przebudowie P4.2.2), z `Footer.tsx` (już w P4.0.2) oraz **nowo dodane z `SeoContentSection.tsx`** (strona główna, sekcja zamykająca CTA) - zweryfikowane na żywo w przeglądarce.
    - Przy okazji naprawiona literówka "słońciem" -> "słońcem" oraz doprecyzowany akapit o karoserii (usunięta fraza "z dożywotnią gwarancją producenta" - myląca, niezwiązana z `facts.md`).
    - Zweryfikowane: `BlogPosting` + `FAQPage` (5 pytań) w DOM, 36 linków wewnętrznych, 0 naruszeń `facts.md` (3 trafienia skanera to poprawne zaprzeczenia), `tsc` i `next build` czyste.
...
- [x] **P4.2.5 - Wypchnij `/naklejki-dla-firm` z pozycji 28,81** ✅ 2026-08-31
    - **Stan zastany był lepszy niż opisany w zadaniu:** wszystkie 5 wymienionych wpisów B2B już linkowało w górę do landingu (dodane przy okazji P4.2.2/P4.2.3), a landing sam linkował w dół do 5 z nich przez `USE_CASES` - hub-and-spoke był już **dwukierunkowy** zanim zacząłem. Stopka (P4.0.2) i `SeoContentSection.tsx` też już linkowały. Checklist z opisu zadania był w pełni spełniony bez dodatkowej pracy.
    - **Realna praca poszła gdzie indziej:** przy weryfikacji linkowania w meta description i H1 landingu znalazłem **6 nowych naruszeń `facts.md`** typu "obietnica doręczenia" - w tym jedno na **stronie potwierdzenia zamówienia**, którego mój skaner nie widział, bo tekst jest rozbity przez JSX (`{" "}` i `<strong>` między słowami zdania rozrywają frazę na fragmenty, których statyczny regex operujący na surowym tekście pliku nie łączy w spójne zdanie).
    - **Naprawione:**
        - `naklejki-dla-firm/page.tsx`: `title`/`description`/`twitter.description` ("odbiór w paczkomacie w 2-3 dni" -> produkcja) + hero-paragraph rozbity przez `{" "}` ("wysyłamy z odbiorem w paczkomacie w **2-3 dni robocze**" -> "produkujemy w **2-3 dni robocze** i wysyłamy do paczkomatu").
        - `alternatywa-dla-sticker-mule-i-stickerapp/page.tsx`: `openGraph.description`, `twitter.description`, FAQ "Jaka jest najlepsza polska alternatywa...", `Product.description` w schema, hero-paragraph rozbity przez `{" "}`, oraz FAQ wprost o czasie dostawy ("wysyłamy... w 2-3 dni robocze" -> produkcja osobno od wysyłki).
        - 🚨 **`zamowienie-sukces/page.tsx` (strona widziana przez KAŻDEGO klienta tuż po zapłaceniu):** etykieta "Produkcja i wysyłka: 2-3 dni robocze" łączyła produkcję z dostawą w jedną liczbę - dokładnie obietnica, której zakazuje `facts.md`, i to w miejscu o najwyższej stawce (świeżo zapłacony klient). Zmienione na "Produkcja: 2-3 dni robocze... a potem wysyłka do paczkomatu" w obu wariantach (przelew/płatność natychmiastowa).
    - **Ograniczenie metody, do zapamiętania:** `blog-agent/audyt-facts.py` czyta surowy tekst pliku - JSX, które rozbija zdanie tagami (`<strong>`) i wstawkami `{" "}`, jest niewidoczne dla regexów działających na jednej linii. Wszystkie 6 naruszeń w tym zadaniu znalazłem **ręcznym czytaniem kodu**, nie skanerem. Przy kolejnym pełnym audycie warto ręcznie przejrzeć strony z rozbudowanym `metadata` (title/description/OG/Twitter) i JSX z `{" "}` w newralgicznych zdaniach o cenie/czasie - to ślepa plama automatycznego narzędzia.
    - Zweryfikowane: `tsc` i `next build` czyste, brak wzorca "odbiór w paczkomacie w N dni" na żywo w przeglądarce na obu landingach.

- [x] **P4.2.6 - Rozbudowany `/slownik-naklejek` jako główny zasób GEO** ✅ 2026-08-31
    - **Stan zastany:** większość wymienionych pojęć (`folia winylowa`, `arkusz A4`, `wlepka`, `etykieta a naklejka`) już istniała w słowniku - zbudowana przy wcześniejszych fazach. Naprawdę brakowało: dedykowanego hasła `naklejki wodoodporne` (dziś top zapytanie klastra materiałowego, 32 wyśw., ale było wyłącznie wzmiankowane wewnątrz definicji folii winylowej, bez własnego `DefinedTerm`) i wariantu pisowni `vlepka` w haśle "Wlepka".
    - **Dopisane:** nowe hasło **"Naklejki wodoodporne"** (własna ikona, definicja z twardą granicą "zmywarka - nie", link do posta B1) oraz zaktualizowane hasło **"Wlepka / vlepka"** (oba warianty pisowni + wzmianka niszy kibicowskiej, link do B4 w treści bloga - patrz niżej). `300 DPI` i `quiet zone` pominięte: pierwsze już pokrywa istniejące hasło DPI (dopisany dopisek "(300 DPI)" w nazwie), drugie odłożone do czasu napisania B5 (QR) - definiowanie terminu bez żadnej powiązanej treści byłoby przedwczesne.
    - **`about`/`mentions` (zaległość P1.5) - zrobione automatycznie, nie ręcznie.** Zamiast ręcznie mapować 32 wpisy do pojęć (koszt utrzymania rósłby z każdym nowym artykułem), stworzono `src/lib/dictionaryTerms.ts` jako **jedno źródło prawdy** dla całego słownika (przeniesione z `slownik-naklejek/page.tsx`) z funkcją `findMentionedTerms()` - dopasowuje pojęcia do treści artykułu tekstowo (po `matchTerms`) i zasila `mentions` w schemacie `Article`/`BlogPosting` w `blog/[slug]/page.tsx` automatycznie, dla wszystkich 32 wpisów naraz, bez utrzymywania osobnej listy. Każde `DefinedTerm` ma teraz stabilny `@id` (`/slownik-naklejek#<slug>`) i realną kotwicę na stronie (`id={slug}`, `scroll-mt-24`).
...
- [x] **P4.2.9 (NOWE) - Pełny audyt zgodności z `facts.md`** ✅ 2026-08-30 - przeskanowane **wszystkie 31 wpisów i 12 landingów** wg 13 kategorii zakazów z `facts.md`. Skaner: `blog-agent/audyt-facts.py` (uruchamiaj po każdej większej partii treści).
    ...
    - 🚨 **ZNALEZIONA PRZYCZYNA ŹRÓDŁOWA:** wytyczna GEO w tym pliku (§1) **nakazywała** eksponować "paczkomat w 72h" w **KAŻDYM** wpisie - wprost wbrew `facts.md`, który zakazuje deklarowania całkowitego czasu doręczenia. Dlatego obietnice wracały mimo wcześniejszej naprawy (patrz notatka przy wpisie A4). Wytyczna poprawiona na język produkcji. Przy okazji `rules.md` §3 miał przykład "Realizacja w 3 dni robocze" niezgodny z ustalonym "produkcja 2-3 dni robocze" - poprawione.
    - **Czyste kategorie (0 naruszeń):** gwarancja skanowalności QR, liczba sztuk na A4 jako gwarancja, ekspozycja NASZEGO generatora AI (HOLD respektowany), "zaprojektuj naklejkę/grafikę", materiały nieoferowane, security/void, darmowa dostawa i rabaty hurtowe, repozycjonowalność - wszystkie wystąpienia to poprawne zaprzeczenia.
    - **Stan po audycie:** kategoria "obietnica doręczenia" = **0 trafień** w całym serwisie.

- [x] **P4.2.10 (NOWE) - Domknięcie luki w audycie: `src/components/**` nie było skanowane** ✅ 2026-08-31 - poprzedni audyt (P4.2.9) skanował tylko `src/content/blog/*.md` i `src/app/**/page.tsx`. **Pominął cały `src/components/`** - w tym `SeoContentSection.tsx`, czyli treść SEO **strony głównej** (859 wyświetleń, CTR 7,33% - najlepiej pracująca powierzchnia serwisu). Skaner rozszerzony na `src/app/**/*.tsx` i `src/components/**/*.tsx`.
    - **Znalezione i naprawione 3 naruszenia w `SeoContentSection.tsx`:** "ekspresowym drukiem w 72h" (obietnica doręczenia) -> "wyprodukujemy naklejki w 2-3 dni robocze"; statystyka "72h / Czas realizacji" -> "2-3 dni / Czas produkcji"; "naklejasz raz, na lata" (sufit trwałości) -> "naklejasz raz, bez możliwości przełożenia w inne miejsce".
    - Rozszerzenie zakresu ujawniło też szum: dosłowne dopasowania słowa TS `void` w kodzie aplikacji (30 fałszywych trafień w `src/components/creator/`, `account/`, `auth/`) - regex kategorii H zawężony do wymogu kontekstu "security"/"VOID" w cudzysłowie.
    - Zweryfikowane na żywym serwerze (nie w plikach źródłowych): strona główna nie zawiera już "72h" ani "na lata".
...
- [x] **P4.2.12 (NOWE) - Przestawienie `/naklejki-dla-firm` na długi ogon + odświeżenie meta dwóch wpisów** ✅ 2026-09-24 (commit `131d00a`)
    - **`/naklejki-dla-firm` z głowy `naklejki firmowe` na `naklejki dla małych firm`:** title `Naklejki dla małych firm - od 1 arkusza, 49 zł, faktura VAT` (59 zn.), H1 `Naklejki dla małych firm z własnym logo`, OG/Twitter i `WebPage.name` w JSON-LD zgodne z H1. Nowa sekcja H2 `Naklejki dla małej firmy - jak zacząć od jednego arkusza` (akapit z linkiem do wpisu o małym nakładzie + 3 karty: kilka wzorów na arkuszu, kilka arkuszy w jednej dostawie, arkusz próbny - wszystko z potwierdzonych faktów, m.in. dostawa 19,99 zł liczona raz za zamówienie). 3 nowe FAQ na początku listy (12 łącznie, widok = schemat, zweryfikowane w DOM). `dateModified` 2026-09-24 + widoczna etykieta "Ostatnia aktualizacja" (jak na pozostałych landingach - bez niej Google dyskontuje `dateModified`) + `lastModified` w `sitemap.ts`. URL, breadcrumb i wezwanie "naklejki dla firm" w treści zostają, więc głowa nie jest porzucona, tylko schowana za kwalifikowanym ogonem.
    - **`naklejka-ze-zdjecia-jak-przeniesc-wspomnienia-na-naklejke`:** pod frazę `jak zrobić naklejkę ze zdjęcia` wg formuły z P4.0.1 - title `Jak zrobić naklejkę ze zdjęcia - od 1 szt., 49 zł/A4` (52 zn.), description 157 zn. Link w górę do `/fotonaklejki` już istniał w 2. akapicie; przeniesiony do 1. akapitu (anchor "fotonaklejką", zweryfikowany w surowym SSR HTML). Bez bumpa `updated` (meta + przeniesienie linku).
    - **`wklepki-i-wlepy-z-wlasnym-nadrukiem-dla-artystow-i-spolecznosci`:** title `Wlepki z własnym nadrukiem dla artystów i fanów, 49 zł/A4` (57 zn.), description 151 zn. Odróżnik w tytule = kąt informacyjny (artyści, fani), żeby nie ścigać się z transakcyjnym landingiem `/wlepki-na-zamowienie` (rozdział intencji z B2). **`updated` celowo nietknięte (2026-09-10).**
    - **`llms.txt` / `llms-full.txt` przebudowane;** wpis strony w `PAGES` w `scripts/generuj-llms-txt.mjs` dopasowany do nowego pozycjonowania (był na sztywno).
    - **Weryfikacja:** `tsc --noEmit` czysty, `audyt-facts.py` bez nowych trafień, dev + produkcja (title, H1, `dateModified`, `llms-full.txt`). Uwaga: `<title>` wpisów ma sufiks ` - Blog MałeNaklejki` (konwencja `blog/[slug]/page.tsx`), więc w SERP realnie ~70 zn. - fraza stoi na początku.
    - **Punkt startowy do porównania w GSC (przed zmianą):** landing 109 wyśw. / 0 klik. / poz. 28,81 (stan z P4.2.5), `naklejki firmowe` poz. 39,9; wpis o wlepkach 24 klik. / 1 323 wyśw. Sprawdź CTR i pozycje **po 3-4 tygodniach (ok. 20.10.2026)**. Jeśli wpis o wlepkach straci klik. mimo wzrostu wyświetleń, wróć do wersji z `od 1 szt.` w tytule. **Do ręcznego kroku właściciela:** prośba o ponowną indeksację `/naklejki-dla-firm` w GSC.
