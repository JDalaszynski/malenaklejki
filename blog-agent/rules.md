...
* **Pisania o "projektowaniu" naklejki/grafiki w kreatorze (KATEGORYCZNY ZAKAZ - dotyczy też generatora AI):**
  Kreator nie służy do projektowania naklejek ani grafiki. Kreator służy do **projektowania arkusza** z naklejkami: wgrywania gotowego obrazu, kadrowania go, wyboru kształtu cięcia oraz układania i konfiguracji całego arkusza A4. Nie posiada ręcznego edytora tekstu, dodawania kształtów ani biblioteki grafik do składania projektu "klocek po klocku".
  Doradzając użytkownikowi, jak przygotować taką grafikę, **ZAWSZE na pierwszym miejscu proponuj użycie ZEWNĘTRZNYCH generatorów obrazów AI (takich jak Midjourney, ChatGPT, Gemini)** (opis tekstowy → gotowy obraz), a dopiero w dalszej kolejności wymieniaj zewnętrzne darmowe programy takie jak Canva, Word czy PowerPoint. **Nie eksponuj natomiast NASZEGO wbudowanego generatora AI** - jego promocję właściciel nadal wstrzymuje (aktualizacja 2026-08-04; patrz pamięć `ai-generator-content-hold`, `keywords.md` §7 i `strategy.md` §6). Wymieniaj wyłącznie zewnętrzne narzędzia. 
  Pamiętaj, że generowanie obrazu przez AI to nie jest "projektowanie w kreatorze" - to alternatywne źródło gotowej grafiki (obraz trafia potem do kreatora dokładnie tak samo jak wgrane zdjęcie czy plik z Canvy). Nigdy nie używaj słowa "projektować"/"zaprojektuj" w odniesieniu do samej naklejki lub grafiki (także w kontekście AI) - to słowo rezerwujemy wyłącznie dla układania arkusza.
  ...
  * ✅ DOBRZE (generatory AI jako pierwsze źródło grafiki, nie jako "projektowanie"): "Nie masz gotowego zdjęcia ani grafiki? Wygeneruj obraz podając prosty opis tekstowy w narzędziach AI takich jak Midjourney, ChatGPT, Gemini, albo stwórz prosty projekt w darmowym programie jak Canva lub Word. Gotowy obraz po prostu wgraj do naszego kreatora - a my zajmiemy się automatycznym wycięciem po obrysie!"
---
...
* **Optymalizacja wagi zdjęć (obowiązkowo, bez przypominania):** Zdjęcia wgrywane przez użytkownika bywają ciężkie (często są to pliki PNG zapisane z rozszerzeniem `.jpg`, po ~2 MB każdy). Zawsze - przy każdym artykule - przekonwertuj je na realny JPEG i skompresuj bez widocznej utraty jakości, zanim osadzisz je w treści (sharp: `.jpeg({ quality: 88, mozjpeg: true })`; rozdzielczość ~1024 px wystarcza na bloga). Docelowa waga to ~100-250 KB na plik zamiast ~2 MB. Kompresję rób na etapie nadawania nazw SEO, przed generowaniem Pinów i przed dodaniem paska z logo.